...
## Overview
 - In this lab:
1- You’ll create a new Terraform Workspace named ivolve.
2- Use modules to define reusable Terraform configurations for creating:
 - VPC
 - Subnets
 - Load Balancer (LB)
 - EC2 Instances
...
3- Outputs will include:
 - Public IP of the Load Balancer (LB)
4- Validate the Load Balancer by accessing its public IP via a browser and capturing a screenshot.
## Prerequisites
1- Terraform Installed: Ensure Terraform is installed and configured on your system.
2- AWS CLI Configured: AWS CLI with proper credentials configured for access to AWS services.
3- IAM Role with Permissions: Permissions for creating VPC, EC2, Load Balancer, Subnets, Security Groups, and related resources.
4- Text Editor/IDE: VS Code or similar for editing Terraform files.
## Steps to Implement
### Step 1. First, we will start with Variables File
```
...
```
### Step 3 : Move to main infrastructure file using modules
```
...
```
### Step 4 : make the Terraform Backend file
```
...
```
### Step 5 : Create a new workspace called ivolve
```
...
```
### Step 7 : Loadbalancer Module variable file
```
...
```
### Step 8 : Subnet Module variable file
```
...
```
### Step 9 : VPC Module Variable file
```
...
```
### Step 10 : Finally Let's Apply this Configuration
```
...
2- An Application Load Balancer accessible via public IP.
3- Two EC2 instances (one in each subnet).
4- NAT Gateway managed by Terraform.
5- Terraform Outputs:
 - Public Load Balancer IP
 - Public and private IPs of EC2 instances.
6- Screenshot of the Load Balancer's accessible public page.
# 🙏 Thank You
